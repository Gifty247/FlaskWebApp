...
def model_inference(image_tensor):
    with torch.no_grad():  # No need to compute gradients
        ...
        print("Unique values in output:", np.unique(output))
    return output

def create_output_image(original_image, prediction, threshold=0.5):
    lane_mask = (prediction > threshold).astype(np.uint8)  # Threshold predictions to create a binary mask
    lane_mask = cv2.resize(lane_mask, (original_image.shape[1], original_image.shape[0]), interpolation=cv2.INTER_NEAREST)
    colored_mask = np.zeros_like(original_image)
    colored_mask[:, :, 0] = lane_mask * 255   # Red mask
